$(document).ready(function(){
	...
	let jlhsoal=0;
	function tampil(){
		let url = "../../json/consojangkauan.json";
		$.getJSON(url, function(data){
			console.log(i);
			jlhsoal = data.consojangkauan.length;
			$("#soalmodus").html(data.consojangkauan[i].soal);
			$("#datamodus").html(data.consojangkauan[i].data);
			$("#soalmodus2").html(data.consojangkauan[i].soal2);
			$("#pembahasanmean").html(data.consojangkauan[i].pembahasan);
			$("#noconso").html(i+1);
...
$(document).ready(function(){
	function tampil2(){
        let url = "../../json/latihan_jangkauan.json";
